...
namespace CDT.Akamai.EdgeAuth
{
    ...
    ///     <see cref="#ExtensionMethods.ComputeKeyedHash(Stream, KeyedHashAlgorithm" />
    ///     Currently supported hashes include MD5; SHA1; SHA256
    ...
    public static class ExtensionMethods
    {
        ...
        public static byte[] ReadExactly(this Stream stream, long maxCount)
        {
            using (var result = new MemoryStream())
            {
                ...
                while ((bytesRead = stream.Read(buffer, 0,
                    leftToRead > int.MaxValue ? int.MaxValue : Convert.ToInt32(leftToRead))) != 0)
                {
        ...
        /// </summary>
        /// <param name="data">
        ///     a byte array to encode. The assumption is that the string to encode
