...
namespace CDT.Akamai.Tests
{
    ...
    public class ClientCredentialTest
    {
        ...
        {
            Assert.ThrowsException<ArgumentNullException>(() => new ClientCredential(null, _accessToken, _secret));
        }

        [TestMethod]
        public void ConstructorDefaultTest_NullAccessToken()
        {
            Assert.ThrowsException<ArgumentNullException>(() => new ClientCredential(_clientToken, null, _secret));
        }
